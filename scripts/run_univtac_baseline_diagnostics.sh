...
RUN_DIR="${RUN_DIR:-outputs/univtac_rgb_joint_20260904_121012}"
...
case "${RUN_MODE}" in
    smoke)
        ...
        ;;
    full)
        CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-500 1000 1500 2000}"
...
if (( ${#checkpoint_steps_array[@]} == 0 || ${#trajectory_ids_array[@]} == 0 || ${#seeds_array[@]} == 0 || ${#conditions_array[@]} == 0 )); then
...
if ! nvidia-smi --query-gpu=index --format=csv,noheader | tr -d ' ' | grep -Fxq "${EVAL_GPU}"; then
...
for required_metadata in meta/info.json meta/episodes.jsonl meta/tasks.jsonl meta/modality.json meta/stats.json meta/relative_stats.json; do
...
CUDA_VISIBLE_DEVICES="${EVAL_GPU}" uv run python -c \
...
CUDA_VISIBLE_DEVICES="${EVAL_GPU}" uv run python \
    examples/UniVTAC/evaluate_baseline_diagnostics.py "${diagnostic_args[@]}"
