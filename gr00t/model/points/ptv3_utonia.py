...
class Point3DRoPE(nn.Module):
    def __init__(self, head_dim, base=10000):
        super().__init__()
        assert head_dim % 3 == 0, f"Head dimension must be divisible by 3 for 3D RoPE, {head_dim}"
    ...
    def get_cos_sin(self, xyz):
        frequencies = self.inv_freq.unsqueeze(0)
        axis_embeddings = []
        for axis in range(3):
            embedding = xyz[:, axis : axis + 1] * frequencies
            axis_embeddings.append(torch.cat((embedding, embedding), dim=-1))
        embedding_3d = torch.cat(axis_embeddings, dim=-1)
        return embedding_3d.cos().unsqueeze(1), embedding_3d.sin().unsqueeze(1)

    @staticmethod
    def rotate_half(x):
        midpoint = x.shape[-1] // 2
        first_half, second_half = x[..., :midpoint], x[..., midpoint:]
    ...
    def forward(self, q, k, xyz):
        ...
        for axis in range(3):
            rotated_q.append(
                q_chunks[axis] * cos_chunks[axis]
                + self.rotate_half(q_chunks[axis]) * sin_chunks[axis]
            )
            rotated_k.append(
                k_chunks[axis] * cos_chunks[axis]
                + self.rotate_half(k_chunks[axis]) * sin_chunks[axis]
            )
        return torch.cat(rotated_q, dim=-1), torch.cat(rotated_k, dim=-1)
...
class SerializedAttention(nn.Module):
    ...
    def forward(self, point):
        ...
        qkv = self.qkv(point.feat)[order].reshape(-1, 3, num_heads, channels // num_heads)
        q, k, v = qkv.unbind(dim=1)
        q, k = self.rope(q, k, point.coord[order].clone())

        if not self.enable_flash:
            q = q.reshape(-1, patch_size, num_heads, channels // num_heads).permute(0, 2, 1, 3)
            k = k.reshape(-1, patch_size, num_heads, channels // num_heads).permute(0, 2, 1, 3)
            v = v.reshape(-1, patch_size, num_heads, channels // num_heads).permute(0, 2, 1, 3)
