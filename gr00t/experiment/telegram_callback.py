...
"""Hugging Face callback and message formatting for Telegram notifications."""
...
class TelegramTrainingCallback(TrainerCallback):
    """Send rank-zero-only start, checkpoint, finish, and failure messages."""
    ...
    def _send(self, message: str) -> None:
        try:
            ...
            # A custom notifier must not be able to interrupt training either.
            logger.warning(
                "Telegram notification callback failed (%s); training will continue.",
                type(exc).__name__,
            )
    ...
    def _step_lines(args: Any, state: Any, *, label: str = "Step") -> list[str]:
        ...
        max_steps = TelegramTrainingCallback._max_steps(args, state)
